...
export function createStatusBarItem(): vscode.StatusBarItem {
	if (!statusBarItem) {
		...
		statusBarItem.command = "codeMirrorCast.statusBarMenu";
	}
  ...
  if (!statusBarItem) throw new Error(`Unhandled status: ${status}`);
	switch (status) {
		case ServerStatus.Waiting:
			...
			statusBarItem.text = `$(pulse) Waiting... : ${port}`;
			statusBarItem.tooltip = "Clique pour arrêter le serveur";
			break;
		case ServerStatus.Started:
			// console.log("Server has started.");
      statusBarItem.text = `$(mirror) Server Started : ${port}`;
			statusBarItem.tooltip = "Clique pour arrêter le serveur";
			break;
		case ServerStatus.Stopped:
			// console.log("Server is stopped.");
       statusBarItem.text = `$(debug-disconnect) Server Stopped : ${port}`;
